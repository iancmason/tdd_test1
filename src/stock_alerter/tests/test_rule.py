...
class PriceRuleTest(unittest.TestCase):
    ...
    def setUpClass(cls):
      goog = Stock("GOOG")
      goog.update(datetime(2020, 2, 10), 11)
...
class AndRuleTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        goog = Stock("GOOG")
        goog.update(datetime(2020, 2, 10), 8)
        goog.update(datetime(2020, 2, 11), 10)
        goog.update(datetime(2020, 2, 12), 12)
        msft = Stock("MSFT")
        msft.update(datetime(2020, 2, 10), 10)
        msft.update(datetime(2020, 2, 11), 11)
        msft.update(datetime(2020, 2, 12), 12)
        redhat = Stock("RHT")
        redhat.update(datetime(2020, 2, 10), 7)
        cls.exhcange = {"GOOG": goog, "MSFT": msft, "RHT": redhat}
